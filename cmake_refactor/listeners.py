...
class TargetInputListener(CMakeListener):
    ...
    def exitAdd_library(self, ctx: CMakeParser.Add_libraryContext):
        ...
        files = [f.replace("${CMAKE_CURRENT_LIST_DIR}/", base_path) for f in files]
        files = self.remove_cmake_var(files)
        files = [os.path.join(base_path, f) for f in files if not os.path.dirname(f)]
        ...
        if node is None:
            node = TargetNode(target, headers, sources, cml_path=base_path)
